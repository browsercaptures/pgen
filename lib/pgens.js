...
const modulePath = path.join("node_modules", "pgen")

const sourcePath = fs.existsSync(modulePath) ? modulePath : path.join(__dirname, "..")
...
const { version } = require(path.join(sourcePath, 'package.json'))
...
class App{
	...
	async create(name){
		...
		fs.copyFileSync(path.join(sourcePath, "LICENSE"), `${pkgPath}/LICENSE`)
		...
		fs.copyFileSync(path.join(sourcePath, `s/p`), `${pkgPath}/s/p`)
		
		fs.copyFileSync(path.join(sourcePath, `s/publish`), `${pkgPath}/s/publish`)
		
		fs.copyFileSync(path.join(sourcePath, `s/bump.js`), `${pkgPath}/s/bump.js`)
